...
class Dataset(object):
    ...
    def __init__(self, rootDB, subsetName, yrLst):
        ...
        self.yrLst = yrLst
        self.time = funDB.readDBtime(rootDB=self.rootDB,
...
class DatasetLSTM(Dataset):
    ...
    def __init__(self,
        ...
        if isinstance(var[0], list):
            ...
            self.varInputTs = funDB.readVarLst(rootDB=self.rootDB,
                                               varLst=var[0])
        if isinstance(var[1], list):
            self.varInputConst = var[1]
        else:
            self.varInputConst = funDB.readVarLst(rootDB=self.rootDB,
                                                  varLst=var[1])
    ...
    def readTarget(self):
        ...
        data = funDB.readDataTS(rootDB=self.rootDB,
                                rootName=self.rootName,
                                indSub=self.indSub,
                                indSkip=self.indSkip,
                                yrLst=self.yrLst,
                                fieldName=self.varTarget,
                                nt=nt,
                                ngrid=ngrid)
    ...
    def readInput(self):
        ...
        for var in self.varInputTs:
            dataTemp = funDB.readDataTS(rootDB=self.rootDB,
                                        rootName=self.rootName,
                                        indSub=self.indSub,
                                        indSkip=self.indSkip,
                                        yrLst=self.yrLst,
        ...
        for var in self.varInputConst:
            dataTemp = funDB.readDataConst(rootDB=self.rootDB,
                                           rootName=self.rootName,
                                           indSub=self.indSub,
                                           indSkip=self.indSkip,
                                           yrLst=self.yrLst,
                                           fieldName=var,
                                           ngrid=ngrid)
            statTemp = funDB.readStat(rootDB=self.rootDB,
                                      fieldName=var,
                                      isConst=True)
            dataNormTemp = (dataTemp - statTemp[2]) / statTemp[3]
            data[:, :, k] = np.repeat(np.reshape(dataTemp, [ngrid, 1]),
                                      nt,
                                      axis=1)
            dataNorm[:, :, k] = np.repeat(np.reshape(dataNormTemp, [ngrid, 1]),
                                          nt,
                                          axis=1)
            stat[:, k] = statTemp
            k = k + 1
...
class DatasetPost(Dataset):
    ...
    def readPred(self,
                 *,
                 ...
                 reTest=False,
                 epoch=None):
        bPred = funLSTM.checkPred(out=out,
                                  rootOut=rootOut,
                                  test=self.subsetName,
                                  drMC=drMC,
                                  epoch=epoch,
                                  syr=self.yrLst[0],
                                  eyr=self.yrLst[-1])
    ...
    def statCalSigma(self, *, field='LSTM'):
        ...
        statSigma = classPost.statSigma(dataMC=dataPredBatch,
                                        dataSigma=dataSigma,
    ...
    def statRegSigma(self, dsReg, *, field='LSTM', opt=1, fTest=None):
        ...
        model = statSigma.regComb(dsReg, field=field, opt=opt, fTest=fTest)
        setattr(self, 'statSigma_' + field, statSigma)
        return statSigma, model

    def statRegSigma2(self,
                      dsReg,
                      *,
                      predField='LSTM',
                      targetField='SMAP',
        ...
        model = statSigma.regComb2(dsReg,
                                   predField='LSTM',
                                   targetField='SMAP',
                                   opt=opt,
                                   fTest=fTest)
        setattr(self, 'statSigma_' + predField, statSigma)
        return statSigma, model

    def statCalConf(self,
                    *,
                    predField='LSTM',
                    targetField='SMAP',
                    rmBias=False):
        ...
        if hasattr(self, 'statSigma_' + predField):
            ...
            statSigma = self.statCalSigma(field=predField)
        statConf = classPost.statConf(statSigma=statSigma,
                                      dataPred=dataPred,
    ...
    def statCalProb(self, *, predField='LSTM', targetField='SMAP'):
        ...
        if hasattr(self, 'statSigma_' + predField):
            ...
            statSigma = self.statCalSigma(field=predField)
        statProb = classPost.statProb(statSigma=statSigma,
                                      dataPred=dataPred,
                                      dataTarget=dataTarget,
                                      dataMC=dataMC)
        return statProb
