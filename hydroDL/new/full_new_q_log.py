...
for iEp in range(1, nEp + 1):
    ...
    optim.zero_grad()
    yP = model(xT)
    yP2 = F.conv1d(yP.exp().permute([1, 2, 0]),
                   qcT).permute(2, 0, 1).add(sn).log()
...
# torch.save(model, 'temp_log')
model = torch.load('temp_log')
...
yF2 = F.conv1d(yF.exp().permute([1, 2, 0]), qcT).permute(2, 0, 1).add(sn).log()
...
for k in range(nq):
    qMat[:, k] = np.convolve(np.flip(qc[:, k]), yG[:, k], 'valid')

# ax.plot(df.index[364:], b, color=cLst[k])
b = np.sum(qMat, axis=1)
# c = transform.transOutAll(b[:, None], mtdY, statY)
fig, ax = plt.subplots(1, 1, figsize=(16, 6))
for k in range(nq):
    ax.plot(df.index[364:], qMat[:, k], color=cLst[k])
