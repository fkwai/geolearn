...
def readAttrAll(*, saveDict=False):
    dataFolder = os.path.join(
        dirDB, "camels_attributes_v2.0", "camels_attributes_v2.0")
...
def readSMAP(varLst, usgsIdLst):
    ...
    for ivar in range(len(varLst)):
        x[:, :, ivar] = (
            x[:, :, ivar] * smapstaDict[varLst[ivar]][3] +
...
class DataframeCamels(Dataframe):
    ...
    def getHour(self, *, doNorm=True, rmNan=True):
        ...
        C, ind1, ind2 = np.intersect1d(
            self.time, thourLst, return_indices=True)
        data = data[:, ind2, :]
        return data
    ...
    def getCSV(
        ...
        *,
        ...
        C, ind1, ind2 = np.intersect1d(
            readtLst, tcsvdataLst, return_indices=True)
        data = data[:, ind2, :]
        if doNorm is True:
