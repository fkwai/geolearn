...
def optimally_align_text(
    ...
    for i, text in enumerate(texts):
        ...
        for h, v in alignment:
            ...
            intersections = sum(
                [abs(b.width * b.height)
                 if b is not None else 0 for b in intersections]
            )
    ...
    **kwargs
    ...
    for i in range(lim):
        ...
        if avoid_points:
            ...
            d_x_points, d_y_points, q2 = [0] * \
                len(texts), [0] * len(texts), (0, 0)
        ...
        if text_from_objects:
            ...
            d_x_objects, d_y_objects, q3 = [
                0] * len(texts), [0] * len(texts), (0, 0)
        ...
        move_texts(texts, dx, dy, bboxes=get_bboxes(
            texts, r, (1, 1), ax), ax=ax)
        if save_steps:
            if add_step_numbers:
                plt.title(i + 1)
            plt.savefig(
