...
def plotMultiBoxFig(data,
                    ...
                    ylabel=None):
    nc = len(data)
    fig, axes = plt.subplots(ncols=nc,
                             sharey=sharey,
                             figsize=figsize,
                             constrained_layout=True)
    nv = len(data[0])
...
def plotlocmap(lat,
    ...
    if ax is None:
        ...
        ax = fig.subplots()
    mm = basemap.Basemap(llcrnrlat=min(np.min(baclat), np.min(lat)) - 0.5,
