...
for kkk in range(0, 3):
    ...
    if kkk == 2:
        hucStrLst = ['02', '05', '17']  # [ref, close, far]

    hucLst = np.asarray(hucStrLst, dtype=int)-1
    saveFolder = os.path.join(
        rnnSMAP.kPath['dirResult'], 'paperSigma', 'ecoSingle')
    ...
    if 'loadData' in doOpt:
        ...
        for k in range(0, len(hucLst)):
            trainName = 'ecoRegion'+str(hucLst[0]+1).zfill(2)+'_v2f1'
            trainName = 'ecoRegion'+str(hucLst[k]+1).zfill(2)+'_v2f1'
