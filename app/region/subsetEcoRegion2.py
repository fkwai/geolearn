...
for case in caseLst:
    ...
    setL0 = set(range(len(lat)))
    indL3 = list(setL3)
    indL2 = list(setL2)
    indL1 = list(setL1-setL2)+indL3
    indL0 = list(setL0-setL1)+indL3    
    df.subsetInit('ecoReg_{}_L3_v2f1'.format(case), ind=indL3)
    df.subsetInit('ecoReg_{}_L2_v2f1'.format(case), ind=indL2)
    df.subsetInit('ecoReg_{}_L1_v2f1'.format(case), ind=indL1)
    df.subsetInit('ecoReg_{}_L0_v2f1'.format(case), ind=indL0)
    ...
    # plot.plotMap(data, lat=lat, lon=lon, ax=ax, cbar=False,title=case)
    # fig.show()
