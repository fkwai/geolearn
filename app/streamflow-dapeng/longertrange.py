...
exp_disp='change_basinnorm/change_trange'
...
# nDayLst = [1,3,7]
# for nDay in nDayLst:
#     optData = default.update(default.optDataCamels, daObs=nDay, tRange=[19850101, 19950101])
#     optModel = default.update(default.optLstm, hiddenSize=HIDDENSIZE)
#     optLoss = default.optLossRMSE
#     optTrain = default.update(default.optTrainCamels, miniBatch=[BATCH_SIZE, RHO], nEpoch=EPOCH)
#     save_path = exp_name + '/' + exp_disp + \
...
predLst = list()
for out in outLst:
    df, pred, obs = master.test(out, tRange=tRange, subset=subset, basinnorm=True, epoch=200, reTest=False)
