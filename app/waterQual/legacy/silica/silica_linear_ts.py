...
for k, siteNo in enumerate(siteNoLst):
    ...
    dfP2 = wqLinear.loadSeq(siteNo, code, 'ARMA',
                            optT=optT, order=(5, 0, 0))
    rmseARMA, corrARMA = waterQuality.calErrSeq(
        dfP2[code], dfObs[code])
...
axes[0].plot(rmseMat[:,0,0],rmseMat[:,2,0],'*')
axes[0].plot([0,10],[0,10],'-k')
axes[0].set_xlim(0,10)
axes[0].set_ylim(0,10)
axes[1].plot(corrMat[:,0,0],corrMat[:,2,0],'*')
...
def funcPoint(iP, axP):
    ...
    styLst = '-*'
    dfPLst = [dfP1, dfP2, dfP3]
    rmseLst = [rmse1, rmse2, rmse3]
    corrLst = [corr1, corr2, corr3]
...
for ax in figP.axes:
    ax.set_xlim(np.datetime64('2010-01-01'), np.datetime64('2020-01-01'))
figP.canvas.draw()

for ax in figP.axes:
    ax.set_xlim(np.datetime64('1990-01-01'), np.datetime64('2000-01-01'))
figP.canvas.draw()
