...
label = 'QFP_C'
labLst2 = ['train end point', 'train - sequence',
           'test end point', 'test - sequence']

corrMat = np.full([nSite, len(codeLst), 2*len(dataNameLst)], np.nan)
rmseMat = np.full([nSite, len(codeLst), 2*len(dataNameLst)], np.nan)
for iData, dataName in enumerate(dataNameLst):
    ...
    for iT, subset in enumerate([trainSet, testSet]):
        ...
        for iCode, code in enumerate(codeLst):
            ...
            for siteNo in dictSite[code]:
                ...
                corrMat[iS, iCode, iT+iData*2] = corr
                rmseMat[iS, iCode, iT+iData*2] = rmse
