...
cp1 = (64, 3, 2)
cp2 = (128, 5, 2)
model = cnn.Cnn1d(nx=nx, nt=rho, cnnSize=1, cp1=cp1, cp2=cp2).cuda()
...
for k in range(rho, nt):
    yF = model(xF[k-rho:k, :, :])
    dfYP.at[dfYP.index[k], 'CNN'] = yF.detach().cpu().numpy().flatten()[0]
...
t1 = t[ind1]
t2 = t[ind2]
v1 = [dfYP[ind1]['CNN'].values.astype(float),
      dfYP[ind1]['WRTDS'].values.astype(float),
      df[ind1][code].values]
v2 = [dfYP[ind2]['CNN'].values.astype(float),
      dfYP[ind2]['WRTDS'].values.astype(float),
      df[ind2][code].values]
rmseWRTDS1, corrWRTDS1 = utils.stat.calErr(v1[1], v1[2])
rmseCNN1, corrCNN1 = utils.stat.calErr(v1[0], v1[2])
rmseWRTDS2, corrWRTDS2 = utils.stat.calErr(v2[1], v2[2])
rmseCNN2, corrCNN2 = utils.stat.calErr(v2[0], v2[2])

# plot
fig, axes = plt.subplots(2, 1, figsize=(16, 6))
axplot.plotTS(axes[0], t1, v1, styLst='--*', cLst='rbk')
axplot.plotTS(axes[1], t2, v2, styLst='--*', cLst='rbk')
axes[0].set_title('site {} WRTDS {:.2f} CNN {:.2f}'.format(
    siteNo, corrWRTDS1, corrCNN1))
axes[1].set_title('site {} WRTDS {:.2f} CNN {:.2f}'.format(
    siteNo, corrWRTDS2, corrCNN2))
# fig.show()
# plot forcing
# ax1 = axes[0].twinx()
# ax2 = axes[1].twinx()
