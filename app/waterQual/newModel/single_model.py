...
class LstmModel(torch.nn.Module):
    def __init__(self, *, nx, ny, hiddenSize, dr=0.8):
        ...
        self.lstm = rnn.CudnnLstm(
            inputSize=hiddenSize, hiddenSize=16, dr=dr)
        self.lstm2 = rnn.CudnnLstm(
            inputSize=16, hiddenSize=hiddenSize, dr=dr)
    ...
    def forward(self, x, doDropMC=False):
        ...
        outLSTM2, (hn, cn) = self.lstm2(F.relu(outLSTM))
        out = self.linearOut(outLSTM2)
        return out


model = LstmModel(nx=nx, ny=ny, hiddenSize=256).cuda()
