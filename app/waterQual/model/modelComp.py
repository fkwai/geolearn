...
if 'plotTsMap' in doLst:
    ...
    iC = 16
    var = varC[iC]
    temp = npfLst[0]['matRmse2'][:, iC]
    indG = np.where(~np.isnan(temp))[0].tolist()

    dataLst = [npf['matRmse2'][indG, iC] for npf in npfLst]
    dataNLst = [npf['matN2'][indG, iC] for npf in npfLst]
    mapTitleLst = [s + 'RMSE' for s in modelLst]

    # dataLst = [npfLst[0]['matRmse2'][indG, iC] - npfLst[2]['matRmse2'][indG, iC],
    #            npfLst[1]['matRmse2'][indG, iC] - npfLst[2]['matRmse2'][indG, iC]]
    # dataNLst = [npf['matN2'][indG, iC] for npf in npfLst]
    # mapTitleLst = ['modelA-modelC', 'modelB-modelC']

    vmin = np.percentile(dataLst[0][~np.isnan(dataLst[0])], 10)
    vmax = np.percentile(dataLst[0][~np.isnan(dataLst[0])], 90)
    vRange = [vmin, vmax]
