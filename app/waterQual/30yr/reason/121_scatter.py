...
gs = gridspec.GridSpec(50, 41)
fig = plt.figure(figsize=[10, 10])
ticks = [-0.5, 0, 0.5, 1]
for k, code in enumerate(codeLst2):
    j, i = utils.index2d(k, 5, 4)
    ax = fig.add_subplot(gs[(j)*10:(j+1)*10, i*10:(i+1)*10])
...
cax = fig.add_subplot(gs[:, 40])
fig.colorbar(sc, cax=cax)
fig.suptitle('corr of LSTM vs WRTDS colored by {}'.format(cVar))
fig.show()
