...
p = np.ndarray(len(siteNoLst))
e = np.ndarray(len(siteNoLst))
for k, siteNo in enumerate(siteNoLst):
    p[k] = dictObs[siteNo]['pr'].mean()  # mm/day
    e[k] = dictObs[siteNo]['etr'].mean()  # mm/day
r = q/a*0.3048**3*24*60*60/10**6
...
def funcM():
    figM, axM = figplot.scatter121Batch(
        xMat, yMat, cMat, labelLst, nXY, optCb=1, cR=cR, ticks=[0, 0.5, 1])
...
def funcP(axP, iP, iM):
    ...
    sc = axP[5].scatter(np.log(q+1), c, c=td, cmap='hsv', vmin=0, vmax=365)
    rIn = np.linspace(0, 1, 11)
    rOut = [0.05, 0.95]
    xIn, yIn, xOut, yOut = analRange(
        np.log(q+1), c, rIn, rOut, bIn=True, bOut=True)
    xp = (xIn[1:]+xIn[:-1])/2
    xe = (xIn[1:]-xIn[:-1])/2
    for i in range(len(rOut)):
        axP[5].errorbar(xp, yOut[:, i], xerr=xe, fmt='-k', capsize=2)
    # yp = (yIn[1:]+yIn[:-1])/2
    # ye = (yIn[1:]-yIn[:-1])/2
    # for i in range(len(rOut)):
    #     axP[5].errorbar(xOut[:, i], yp, yerr=ye, fmt='-g', capsize=2)
...
figM, figP = figplot.clickScatter(funcM, funcP, funcT=funcT)
