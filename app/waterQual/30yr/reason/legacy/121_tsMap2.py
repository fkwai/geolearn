...
# color mat statistics
cMat = np.full([len(siteNoLst), len(codeLst)], np.nan)
for kk, siteNo in enumerate(siteNoLst):
    ...
    for ic, code in enumerate(codeLst):
        ...
        # dfMean1.loc[siteNo][code] = np.nanmean(dfC.iloc[ind2][code].values)
        cMat[kk, ic] = np.nanmean(dfC[code].values)
cVar = 'log-mean'
cMat = np.log(cMat+1)
...
def plotP(xx, yy, cc, iP, code):
    ...
    axplot.mapPoint(axP[2], lat, lon, cc, s=16)
    circle = plt.Circle([lon[iP], lat[iP]], 2, color='black', fill=False)
    ...
    t = dictLSTM[siteNo].index.values
