...
dfT = pd.DataFrame(index=codeLst, columns=['nSite', 'nSample'])
for code in codeLst:
    siteNoCode = dictSite[code]
    nSite = len(dictSite[code])
    nSampleLst = list()
    for siteNo in siteNoCode:
        nSample = np.where(~np.isnan(dictObs[siteNo][code].values))[0].shape[0]
        nSampleLst.append(nSample)
    dfT.at[code, 'nSite'] = nSite
    dfT.at[code, 'nSample'] = int(np.mean(nSampleLst))
