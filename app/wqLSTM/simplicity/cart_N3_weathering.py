...
count = np.nansum(matB[:, :, indC], axis=0)
...
# for a code
for code in codeLst:
    dirSave = r'C:\Users\geofk\work\waterQuality\paper\G200\simplicity\weathering\{}_{}'
    colTemp = colLst.copy()
    ic = codeLst.index(code)
    [mat], indS = utils.rmNan([matQS[:, ic]])
    siteNoCode = [DF.siteNoLst[ind] for ind in indS]    
    latA = dfG.loc[siteNoCode]['LAT_GAGE'].values
    lonA = dfG.loc[siteNoCode]['LNG_GAGE'].values
    for iR in range(3):
        print(iR, 'xxxxxxxxxxxxxxxxxxxxxxxxx', len(colTemp))
        saveFolder = dirSave.format(code, iR)
        if not os.path.exists(saveFolder):
            os.makedirs(saveFolder)
        # plant tree
        dfGC = dfG.loc[siteNoCode][colTemp]
        ...
        strLst, nodeLst, leaf, label = cart.TraverseTree(
            tree, x, Fields=varLst)
        nn = len(nodeLst)
        for k in range(nn):
            ...
            if ix == -2:
                title = 'leaf#{}'.format(k)
                _ = plotLeaf(k, ind,  title)
            else:
                indL = ind[np.where(x[ind, ix] <= th)[0]]
                indR = ind[np.where(x[ind, ix] > th)[0]]
                title = 'node#{} {} < {:.3f}'.format(k, varLst[ix], th)
                _ = plotNode(k, ind, indL, indR, title)
        for ix in np.unique(tree.feature)[1:]:
            colTemp.remove(varLst[ix])
        plt.close('all')
