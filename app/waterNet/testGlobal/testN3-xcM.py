...
DM3.trans(mtdX=mtdX, mtdXC=mtdXC)
dataTup3 = DM3.getData()

np.where(np.isnan(dataTup3[0]))
np.where(np.isnan(dataTup3[1]))
np.where(np.isnan(dataTup3[2]))
...
outDir = os.path.join(saveDir, 'HBN36', 'wn3-ep2000')
...
k = DF.siteNoLst.index('01466500')
fig, ax = plt.subplots(1, 1, figsize=(12, 4))
ax.plot(t, y[:, k], '-k', linewidth=2)
ax.plot(t, yP[:, k], '-r', linewidth=1,
        label='waternet1 {:.2f} {:.2f}'.format(nash1[k], corr1[k]))
ax.plot(t, yL[:, k], '-b', linewidth=1,
        label='LSTM {:.2f} {:.2f}'.format(nash2[k], corr2[k]))
ax.set_title('{}'.format(DF.siteNoLst[k]))
ax.legend()
