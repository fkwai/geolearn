...
import os
from hydroDL.model import trainBasin, crit,  waterNet
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', dest='modelName',
                        type=str, default='WaterNet0119')
    parser.add_argument('-d', dest='dataName', type=str, default='QN90ref')
    args = parser.parse_args()
    modelName = args.modelName
    dataName = args.dataName

# dataName = 'QN90ref'
# modelName = 'WaterNet0119'
...
nr = 5
funcM = getattr(waterNet, modelName)
...
for ep in range(1, 1001):
    ...
    if ep % 50 == 0:
        modelFile = os.path.join(
            saveDir, '{}-{}-ep{}'.format(modelName, dataName, ep))
        torch.save(model.state_dict(), modelFile)

lossFile = os.path.join(saveDir, 'loss-{}'.format(modelName))
