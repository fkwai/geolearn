...
for dataName in dataLst:
    DFC = dbBasin.DataFrameBasin(dataName)
    outName = '{}-{}'.format(dataName, trainSet)
    yL, ycL = basinFull.testModel(
        outName, DF=DFC, testSet=testSet, reTest=True, ep=500, batchSize=100)
    yT = DFC.extractSubset(DFC.q, subsetName=testSet)[:, :, 1]
    indLst = [DFC.siteNoLst.index(x) for x in DF.siteNoLst]
    nash = utils.stat.calNash(yL[:, indLst, 0], yT[:, indLst])
    corr = utils.stat.calCorr(yL[:, indLst, 0], yT[:, indLst])
    nashLst.append(nash)
    corrLst.append(corr)

nashW = utils.stat.calNash(yP, y[nr-1:, :, 0])
corrW = utils.stat.calCorr(yP, y[nr-1:, :, 0])


fig, axes = figplot.boxPlot([nashLst+[nashW], corrLst+[corrW]],
                            label1=['nash', 'corr'],
                            label2=['NLDAS', 'dayMet', 'Maurer', 'waterNet'],
                            yRange=[0, 1])
fig.show()
