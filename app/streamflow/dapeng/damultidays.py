...
for nDay in nDayLst:
    ...
    optData = default.update(default.optDataCamels, daObs=ndayda, davar='streamflow')
    optModel = default.update(default.optLstm, hiddenSize=HIDDENSIZE)
    optLoss = default.optLoss
    optTrain = default.update(default.optTrainCamels, miniBatch=[BATCH_SIZE, RHO], nEpoch=EPOCH)
    save_path = exp_name + '/' + exp_disp + \
...
labelname = ['LSTM']
for nDay in nDayLst:
    labelname.append('DA-'+str(nDay)+'A')
...
fig.show()
# plt.savefig(pathCamels['Out'] + '/' + save_path + "/boxstat_0715.png", dpi=600)
...
#         subtitle = 'LSTM'
#     else:
#         subtitle = 'DA-'+str(nDayLst[ii-1])+'A'
...
stadic_damulti[2] = statDictLst[2]
# write new evaluation results [LSTM, 3, 7, 30, 100, 365]
savedir = os.path.join(pathCamels['Out'], save_path, 'results')
if not os.path.isdir(savedir):
    os.makedirs(savedir)
mFile = os.path.join(savedir, 'evaluation.npy')
np.save(mFile, stadic_damulti)

obsFile = os.path.join(savedir, 'obs.npy')
np.save(obsFile, obs)

predFile = os.path.join(savedir, 'pred.npy')
np.save(predFile, predLst)

nDayLst=[3,7,30,100,365]
daylistFile = os.path.join(savedir, 'daylist.txt')
